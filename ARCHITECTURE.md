...
this file is about how the thing is built and how to change it. (Coding agents
are pointed here from [AGENTS.md](./AGENTS.md).)
...
| Dashboard API + SPA | `src/server/api.ts` | `8080` | Express JSON API that also serves the built React app. |
...
# Run locally (defaults: proxy 3128, dashboard 8080, both non-privileged):
SEP_CONFIG="$(cat config.json)" STATE_DIR=/tmp/sep npm start

# Live frontend dev (separate terminal); it proxies /api to :8080:
npm --prefix dashboard run dev
```

Environment variables: `SEP_CONFIG` (required), `PROXY_PORT` (3128),
`DASHBOARD_PORT` (8080), `STATE_DIR` (/data), `STATIC_DIR` (./dashboard/dist).
...
`dashboard/dist`, then runs `npx tsx src/index.ts`. Exposes `3128` and `8080`.
...
2. Boot with a real `config.json`, then verify routing actually
   changes the egress IP per mode:
   `curl -s -x http://127.0.0.1:3128 https://api.ipify.org` after
